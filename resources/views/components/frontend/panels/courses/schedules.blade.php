    ...
    <div class="container">
        <div class="row mb-4">
            <div class="col-12 text-center">
                <h2 class="text-white m-0">{{ __('frontend.schedules.page_title') }}</h2>
                <p class="text-white-50 m-0">{{ __('frontend.schedules.find_date') }}</p>
            </div>
        </div>
        ...
        <div class="row g-4">
            <div class="col-lg-3 col-md-4">
                <aside class="schedule-sidebar" aria-label="Course filters">
                    <h5 class="text-white mb-3">{{ __('frontend.schedules.course_types') }}</h5>
                    <div class="course-filter-buttons d-grid gap-2">
                        <button class="btn btn-outline-light btn-sm active" data-course="all">
                            {{ __('frontend.schedules.all_courses') }} <span class="badge bg-light text-dark ms-1"
                                id="count-all">0</span>
                        </button>
                        <button class="btn btn-outline-light btn-sm" data-course="d40">
                            {{ __('frontend.schedules.class_d_label') }} <span class="badge bg-primary ms-1"
                                id="count-d40">0</span>
                        </button>
                        <button class="btn btn-outline-light btn-sm" data-course="g28">
                            {{ __('frontend.schedules.class_g_label') }} <span class="badge bg-purple ms-1"
                                id="count-g28">0</span>
                        </button>
                    </div>
                    ...
                    <div class="legend small text-white-50">
                        <div class="d-flex align-items-center mb-2">
                            <span class="legend-dot me-2 bg-primary"></span> {{ __('frontend.schedules.d40_legend') }}
                        </div>
                        <div class="d-flex align-items-center">
                            <span class="legend-dot me-2 bg-purple"></span> {{ __('frontend.schedules.g28_legend') }}
                        </div>
                    </div>
                </aside>
            </div>
                    ...
                    <div class="calendar-header d-flex justify-content-between align-items-center mb-3">
                        ...
                        <div class="calendar-nav">
                            ...
                            <button class="btn btn-primary btn-sm me-2"
                                id="todayBtn">{{ __('frontend.schedules.today') }}</button>
    ...
    <div class="modal-dialog modal-dialog-scrollable">
        <div class="modal-content bg-dark text-light">
            <div class="modal-header border-secondary">
                <h5 class="modal-title" id="dayModalLabel">{{ __('frontend.schedules.modal_schedule') }}</h5>
...
<style>
    ...
    .calendar-head-row>div {
        padding: 12px;
        text-align: center;
        border-right: 1px solid var(--cal-border);
    }

    .calendar-head-row>div:last-child {
        border-right: 0;
    }
    ...
    .calendar-head-row>div:nth-child(7n) {
        border-right: 0;
    }

    .calendar-day.weekend {
        background: var(--cal-cell-alt);
    }
    ...
    .calendar-day:focus {
        outline: 3px solid var(--accent-theme, #22d3ee);
        outline-offset: -3px;
    }
    ...
    document.addEventListener('DOMContentLoaded', async () => {
        ...
        document.querySelectorAll('[data-course]').forEach(btn => {
            btn.addEventListener('click', async function() {
                ...
                await loadCourseSchedules(activeFilter === 'all' ? null : activeFilter
                    .toUpperCase());
            });
        });
    });
