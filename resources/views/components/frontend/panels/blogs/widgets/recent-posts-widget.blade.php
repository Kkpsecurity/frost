...
<div class="sidebar-widget recent-posts-widget mb-4">
    <div class="widget-header">
        <h5 class="widget-title">{{ __('frontend.blog.recent_articles') }}</h5>
    </div>
...
if ($useDefaultImage) {
    $imageUrl = asset('images/Security-Page-1.jpg');
                                ...
                                @else
                                    ...
                                    <div class="recent-post-placeholder d-flex align-items-center justify-content-center bg-light"
                                        style="height: 60px; background: linear-gradient(135deg, #f8f9fa, #e9ecef);">
                        ...
                        <div class="recent-post-content">
                            ...
                            <span
                                class="post-date">{{ $recentPost->published_at ? $recentPost->published_at->format('M j, Y') : $recentPost->created_at->format('M j, Y') }}</span>
                        </div>
                    </div>
                @endforeach
            @endif
