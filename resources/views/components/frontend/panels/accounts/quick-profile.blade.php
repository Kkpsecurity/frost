...
@if ($user)
    ...
    <div class="quick-profile-card">
        <div class="profile-header text-center mb-3">
            ...
            <h5 class="text-white mb-1">{{ __('frontend.home.welcome_back') }}</h5>
            ...
            <div class="row g-2">
                <div class="col-6">
                    <a href="{{ route('dashboard') }}" class="btn btn-primary btn-sm w-100">
                        <i class="fas fa-tachometer-alt me-1"></i>{{ __('frontend.nav.dashboard') }}
                    </a>
                </div>
                <div class="col-6">
                    <form method="POST" action="{{ route('logout') }}" class="d-inline w-100">
                        ...
                        <button type="submit" class="btn btn-outline-light btn-sm w-100">
                            <i class="fas fa-sign-out-alt me-1"></i>{{ __('frontend.nav.logout') }}
                        </button>
                    </form>
                </div>
            </div>
        ...
        <form method="POST" action="{{ route('login') }}" class="quick-login-form">
            ...
            <div class="mb-3">
                <label for="quick-email"
                    class="form-label d-flex justify-start text-white-50 small">{{ __('frontend.auth.email_address') }}</label>
            ...
            <div class="mb-3">
                <label for="quick-password"
                    class="form-label d-flex justify-start text-white-50 small">{{ __('frontend.auth.password') }}</label>
            ...
            <div class="mb-3">
                <div class="form-check">
                    <input type="checkbox" class="form-check-input" id="quick-remember" name="remember"
            ...
            <div class="d-grid mb-3">
                <button type="submit" class="btn btn-primary btn-sm">
                    <i class="fas fa-sign-in-alt me-1"></i>{{ __('frontend.auth.sign_in') }}
                </button>
            </div>
            ...
            <div class="text-center quick-login-links">
                <div class="d-flex justify-content-between">
                    <a href="{{ route('register') }}" class="text-decoration-none">
                        <i class="fas fa-user-plus me-1"></i>{{ __('frontend.auth.create_account') }}
                    </a>
                    @if (Route::has('password.request'))
                        <a href="{{ route('password.request') }}" class="text-decoration-none">
                            <i class="fas fa-question-circle me-1"></i>{{ __('frontend.home.forgot_short') }}
                        </a>
                    @endif
                </div>
            </div>
