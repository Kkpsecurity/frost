                ...
                <div class="p-5 rounded-3"
                    ...
                    <div class="mb-4 pb-3 border-bottom border-secondary">
                        <h1 class="display-5 text-white fw-bold">
                            <i class="fas fa-shield-alt me-3" style="color: var(--frost-accent-color-1);"></i>
                            {{ __('frontend.privacy.heading') }}
                        </h1>
                        <p class="text-white-50 mb-0">
                            <i class="fas fa-calendar-alt me-2"></i>
                            {{ __('frontend.privacy.last_updated') }}: {{ date('F j, Y') }}
                        </p>
                    </div>

                    <div class="policy-intro mb-5">
                        <p class="text-white-50 lead">
                            {{ __('frontend.privacy.intro', ['company' => setting('company_name', config('app.name'))]) }}
                        </p>
                    </div>

                    @foreach (['information_collect', 'how_we_use', 'information_sharing', 'data_security', 'cookies', 'your_rights', 'contact_us'] as $section)
                        <div class="policy-section mb-4">
                            <h3 class="text-white mb-3">
                                <i class="fas fa-chevron-right me-2" style="color: var(--frost-accent-color-2);"></i>
                                {{ __('frontend.privacy.' . $section . '_title') }}
                            </h3>
                            <p class="text-white-50" style="line-height: 1.8;">
                                {{ __('frontend.privacy.' . $section . '_text', ['company' => setting('company_name', config('app.name'))]) }}
                            </p>
                        </div>
                    @endforeach
