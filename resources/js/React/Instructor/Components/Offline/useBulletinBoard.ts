...
export const useBulletinBoard = (): UseBulletinBoardResult => {
    ...
    const fetchTodaysCourses = useCallback(
        async (isBackgroundUpdate = false): Promise<CourseDate[]> => {
            try {
                ...
                // Return the updated course dates
                return courseDates;
            } catch (err) {
    ...
    const refetch = useCallback(async (): Promise<CourseDate[]> => {
        return await fetchTodaysCourses(false);
    }, [fetchTodaysCourses]);
