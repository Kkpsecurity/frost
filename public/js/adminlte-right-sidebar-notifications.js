...
class FrostAdminLTENotifications {
    ...
    setupRightSidebar() {
                ...
                <div id="frost-sidebar-content">
                    ...
                    <ul class="nav nav-tabs" id="sidebar-tabs" role="tablist">
                        <li class="nav-item" role="presentation">
                            <button class="nav-link active" id="messages-tab-btn" data-bs-toggle="tab" data-bs-target="#messages-tab" type="button" role="tab">
                                <i class="far fa-envelope mr-1"></i>Messages
                                <span class="badge badge-danger navbar-badge messages-badge" style="display: none;">0</span>
                            </button>
                        </li>
                        <li class="nav-item" role="presentation">
                            <button class="nav-link" id="notifications-tab-btn" data-bs-toggle="tab" data-bs-target="#notifications-tab" type="button" role="tab">
                                <i class="far fa-bell mr-1"></i>Notifications
                                <span class="badge badge-warning navbar-badge notifications-badge" style="display: none;">0</span>
                            </button>
                        </li>
                    </ul>
                    ...
                    <div class="tab-content" id="sidebar-tab-content">
                        <div class="tab-pane fade show active" id="messages-tab" role="tabpanel">
                            <div class="mt-3">
                                <div id="messages-content">Loading messages...</div>
                            </div>
                        </div>
        ...
        if ($(".control-sidebar").length === 0) {
            ...
            $(".control-sidebar").html(sidebarHtml);
        }
    }

    setupTabSwitching() {
        // Handle tab clicks with proper Bootstrap tab functionality
        $(document).on('click', '[data-bs-toggle="tab"]', function (e) {
            ...
            const tabType = target.replace('#', '').replace('-tab', '');
            if (tabType === 'notifications') {
                window.FrostNotifications.showNotifications();
            } else if (tabType === 'messages') {
                window.FrostNotifications.showMessages();
            }
        });

        // Handle direct tab switching methods
        this.switchToView = (view) => {
            if (view === "notifications") {
                $("#notifications-tab-btn").click();
            } else if (view === "messages") {
                $("#messages-tab-btn").click();
            }
        };
    }
    ...
    showNotifications() {
        ...
        $("#notifications-content").html(
            '<div class="text-center p-3"><i class="fas fa-spinner fa-spin"></i> Loading notifications...</div>'
        );

        // Load data (tab switching is handled by setupTabSwitching)
        this.loadNotifications();
    }
