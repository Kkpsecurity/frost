...
class FrostAdminLTENotifications {
    ...
    updateMessagesBadge() {
        ...
        // Find or create dropdown menu for notifications
        let dropdown = $('#notifications-toggle').next('.dropdown-menu');

        if (dropdown.length === 0) {
            // Convert to dropdown if not already
            $('#notifications-toggle').parent().addClass('dropdown')
                .find('#notifications-toggle').attr('data-toggle', 'dropdown').after(`
                    <div class="dropdown-menu dropdown-menu-lg dropdown-menu-right"></div>
                `);
            dropdown = $('#notifications-toggle').next('.dropdown-menu');
        }
    ...
    updateMessagesDropdown() {
        // Find or create dropdown menu for messages
        let dropdown = $('#messages-toggle').next('.dropdown-menu');

        if (dropdown.length === 0) {
            // Convert to dropdown if not already
            $('#messages-toggle').parent().addClass('dropdown')
                .find('#messages-toggle').attr('data-toggle', 'dropdown').after(`
                    <div class="dropdown-menu dropdown-menu-lg dropdown-menu-right"></div>
                `);
            dropdown = $('#messages-toggle').next('.dropdown-menu');
        }
