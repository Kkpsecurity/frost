...
/**
 * @file VideoCallRequest.php
 * @brief Class for managing video call requests for course dates.
 * @details Provides methods to create, delete, and manage video call requests in Redis.
 */
...
class VideoCallRequest
{
    ...
    public static function Queue(int|string $course_date_id = null): Collection
    {
        ...
        return collect(self::Redis()->hvals(self::RedisKey($course_date_id)))
            ->map(function ($record) {
                return RCache::Unserialize($record);
            })
            ->sortBy('created_at');
    }
