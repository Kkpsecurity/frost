...
/**
 * Format class time in Eastern timezone (8am Eastern always shows as 8am regardless of student location)
 *
 * @param mixed $date Date to format (Carbon instance, string, etc.)
 * @param string $format Format type from RoleManager date formats
 * @return string|null Formatted date string with Eastern timezone or null if date is empty
 */
if (!function_exists('formatClassTime')) {
    function formatClassTime($date, string $format = 'datetime_medium'): ?string
    {
        return \App\Support\RoleManager::formatDate($date, $format, true);
    }
}
